...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    conv_kernel = conv_kernel.repeat(3, 1, 1, 1) #卷积核
    ...
    loss_tensor = torch.zeros_like(lap_blended, device=foreground_img.device)

    loss_tensor[bg_mask]=lap_blended[bg_mask] - lap_fg[fg_mask]

    loss = torch.sum(loss_tensor ** 2) / torch.sum(bg_mask)
