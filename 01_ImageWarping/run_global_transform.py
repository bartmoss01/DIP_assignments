...
def apply_transform(image, scale, rotation, translation_x, translation_y, flip_horizontal):
    ...
    image_new = np.zeros((pad_size*2+image.shape[0], pad_size*2+image.shape[1], 3), dtype=np.uint8) + np.array((255,255,255), dtype=np.uint8).reshape(1,1,3) #填充白色
    ...
    transform_matrix = flip_matrix @ translation_matrix @ rotation_matrix @ scale_matrix
    affine_matrix=transform_matrix[:2, :]
    
    transformed_image=cv2.warpAffine(image, affine_matrix,(w,h), borderValue=(255, 255, 255))
